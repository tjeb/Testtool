...
public interface UserFileMapper {
    ...
    @Select("SELECT * FROM user_files WHERE id = #{id} AND  userId = #{userId}")
    ...
    @Select("SELECT fileName FROM user_files WHERE id = #{id}")
    String getFileNameById(Long id);

    @Select("SELECT * FROM user_files WHERE userId = #{id} ORDER BY addedTime DESC")
    List<UserFile> getByUserId(Long id);

    @Select("SELECT * FROM user_files WHERE userId = #{id} ORDER BY addedTime DESC LIMIT 5")
    ...
    @Select("SELECT * FROM user_files WHERE typeId = #{id}")
    List<UserFile> getAllBytTypeId(Long id);

    @Select("SELECT addedTime FROM user_files WHERE id = #{id}")
